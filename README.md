...
所以 **AgentRouter 的站点 Cookie 无法签到**：Cookie 代表「已经登录」，没有可触发的认证动作，只能用来读额度。要真正签到必须重新认证一次。对于 LinuxDO 渠道注册的账号（无密码），脚本会用 LinuxDO 的 Cookie 重放 OAuth 授权流程：
...
中继成功就是真签到；失败（Cloudflare 拦截、或 LinuxDO 需要手动授权一次）会自动降级为只读额度，通知里明确写「未签到（仅查询额度）」并附上失败原因。
...
- `AGENTROUTER_LINUXDO_COOKIE`：**签到用**。浏览器登录 [connect.linux.do](https://connect.linux.do) 后，请求头中完整的 `Cookie` 值。
- `AGENTROUTER_COOKIE`：**降级用**。浏览器已登录 agentrouter.org 时的完整 `Cookie` 值。中继失败时用它读额度。
- `AGENTROUTER_USER_ID`：可选，请求头 `New-API-User` 的值。

两个 Cookie 至少要配一个。建议都配：优先走中继真签到，失败还能报出额度而不是整个失败。只配 `AGENTROUTER_COOKIE` 的话每次都只会查询额度，不会签到。

> LinuxDO 的 Cookie 权限比站点 Cookie 大，只放进 Actions Secrets，不要写进代码或提交到仓库。
...
- `AGENTROUTER_LINUXDO_COOKIE1`（+ 可选的 `AGENTROUTER_COOKIE1`、`AGENTROUTER_USER_ID1`）
- 依此类推……
